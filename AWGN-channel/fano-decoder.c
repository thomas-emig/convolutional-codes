...
#define DELTA   17.0  // stepsize for threshold adjustment

struct node {
    uint64_t state;
    float metric;
    uint8_t selected_path;         // the (currently) best path (either 0 or 1)
    float transition_metrics[2];   // transition metrics for input 0 and 1
    ...
    float* received_symbol;       // the rx symbol for the corresponding node
};
...
void decoder_destroy(struct decoder** obj) {
    ...
    check_and_delete((void**)&((*obj)->received_symbols));
...
int decoder_init(struct decoder* obj, struct code_param* param) {
    ...
    obj->num_symbols = 1 << obj->parameter.symlen_out;
    ...
    check_and_delete((void**)&(obj->received_symbols));
    obj->received_symbols = malloc(obj->blocklen_w_tail * obj->num_symbols * sizeof(float));
    if (obj->received_symbols == NULL) {
        return -1;
    }
...
void decoder_reset(struct decoder* obj) {
    ...
    // reset received symbols buffer
    memset(obj->received_symbols, 0, obj->blocklen_w_tail * obj->num_symbols);
...
int decoder_input(struct decoder* obj, float* data, uint8_t len) {
    if (len % obj->num_symbols != 0) {
        return -1;
    }
    for (uint8_t count = 0; count < len; count += obj->num_symbols) {
...
// local functions
static int receive_symbol(struct decoder* obj, float* received_symbol) {
    // store received symbol in buffer
    for (int i = 0; i < obj->num_symbols; ++i) {
        obj->received_symbols[(obj->input_counter * obj->num_symbols) + i] = received_symbol[i];
    }
    float* rsym = &(obj->received_symbols[obj->input_counter * obj->num_symbols]);
    ...
    if (cnode->transition_metrics[0] < cnode->transition_metrics[1]) {
        ...
        float metric_tmp = cnode->transition_metrics[0];
    ...
    while (obj->timeout != 0) {
        ...
        float ms = cnode->metric + cnode->transition_metrics[cnode->selected_path];
...
static void get_transition_metric(struct decoder* obj,
    ...
    for (uint8_t in = 0; in < 2; ++in) {
        ...
        transition_metric[in] = 1.0 + obj->parameter.fano_metric_weight * received_symbol[expected_symbol];
